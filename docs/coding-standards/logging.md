...
Create the logger from `ILoggerFactory` using the operation name as the category. This means every log line from that handler is tagged with the operation, making it trivially filterable:
...
Log at the boundary of the handler - called and completed:
...
Log everything from the request that is useful for debugging. If the request has an ID, name, or any discriminating field, it belongs in the scope.
...
See [`CreateAccountEndpoint.cs`](/src/Host/Handlers/Endpoints/Accounts/CreateAccountEndpoint.cs) and [`GetAccountByIdEndpoint.cs`](/src/Host/Handlers/Endpoints/Accounts/GetAccountByIdEndpoint.cs) for full examples.
