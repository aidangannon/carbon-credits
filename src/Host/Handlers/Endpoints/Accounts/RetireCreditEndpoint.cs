...
public static class RetireCreditEndpoint
{
    public static RouteGroupBuilder MapRetireCredit(this RouteGroupBuilder group)
    {
        group
            ...
            .WithDescription("Marks the given credit on the given account as retired, enforcing that it exists and is not already retired");
    ...
    )
    {
        ...
        var credit = serviceResult.Unwrap().Credits.First(c => c.Id == creditId);

        return TypedResults.Ok(credit.ToResponse());
    }
}
